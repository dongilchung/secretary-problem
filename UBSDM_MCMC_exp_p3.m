function UBSDM_MCMC_exp_p3( data_behavior, n_samples, EXP, repeat_number )

output_file = strcat('UBSDM_Exp',num2str(EXP),'_p3_r',num2str(repeat_number),'.mat');
...
mu_temp = [ 92.0293; 0.4894; 11.6292];
mu_temp = repmat( mu_temp, n_subjs + 1, 1 ); % 22 goodSubjs + overall mu

std_temp = [0.7; 0.7; 0.3];

curr_sample = [ mu_temp; std_temp ];
curr_sample = model_to_sample( curr_sample, n_params );
...
log_like_curr = cal_log_like(curr_sample, data_behavior, n_params, n_subjs, subjList, n_trials);
...
for i = 1:n_samples*thinning
    ...
    log_like_prop = cal_log_like(proposal_sample, data_behavior, n_params, n_subjs, subjList, n_trials);
...
B = const.prior*[ 0.7, 0.7, 0.5 ]';
A = 1;
% v = 1;

if sum( std_prior <= eps ) > 0 
    prior_sigma = 0;
else
...
log_prior = log_prior + log( prior_sigma );



function sum_log_likeli = cal_log_like( curr_sample, data_all, n_params, n_subjs, subjList, n_trials )
...
for subj = 1:n_subjs
    ...
    log_likeli_all(subj) = cal_log_likeli_subj_wc( params, data_subjs, n_trials);
