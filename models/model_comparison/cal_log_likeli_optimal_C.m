function [log_likeli_subj, params0 ] = cal_log_likeli_optimal_C( params_subj, data_subj, n_trials )
...
numbers = 1:150;
params0(n_trials-1,1) = 1/2 * max(numbers) - wc;

for i = 1:(n_trials - 2)
    params0(-i+n_trials-1) = (((max(numbers)-params0(-i+n_trials))/max(numbers))...
        *(params0(-i+n_trials)+max(numbers))/2) + params0(-i+n_trials)/max(numbers)*params0(-i+n_trials) - wc;
    % params0(-i+4) = (((max(numbers)-params0(-i+5))/max(numbers))*(params0(-i+5)+max(numbers))/2) + params0(-i+5)/max(numbers)*params0(-i+5) - wc;
end
