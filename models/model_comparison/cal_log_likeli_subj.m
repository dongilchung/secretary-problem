function [log_likeli_subj, threshold, exp_value] = cal_log_likeli_subj( params_subj, data_subj, n_trials )
...
for i = 1:(n_trials-1)
    ...
    exp_u_cum(index) = ((max_v - threshold(index))/(max_v - min_v))*exp_u_trial(index) + ((threshold(index)-min_v)/(max_v - min_v))*exp_u_cum(index + 1);
...
function exp_util = cal_exp_util( min_v, max_v, power_param, exp_v_round, x_vector )
